    ...
    public SqsService() {
        ...
        try {
            ...
            if (!e.awsErrorDetails().errorCode().equals("QueueAlreadyExists")) {
                log.error("Failed to create SQS queue: {}", e.getMessage());
                throw e;
            }
        }
    }
    ...
    public void sendBatchMsg() {
        try {
            ...
            SendMessageBatchRequestEntry entry = SendMessageBatchRequestEntry.builder()
                .id("msg-1")
                .messageBody("Appointment created")
                .build();
            ...
            log.info("Batch message sent successfully to SQS queue: {}", QUEUE_NAME);
        } catch (SqsException e) {
            log.error("Failed to send batch message to SQS: {}", e.getMessage());
            throw e;
        }
    }
}
